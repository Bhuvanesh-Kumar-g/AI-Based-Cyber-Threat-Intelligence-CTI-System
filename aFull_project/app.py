...
sys.path.append(os.path.abspath('deepfake_video'))
sys.path.append(os.path.abspath('deepfake_audio'))
sys.path.append(os.path.abspath('email_phising'))
sys.path.append(os.path.abspath('URL-Detection'))
...
def get_gemini_explanation(analysis_type, verdict, result_data, file_path=None, raw_text=None, context=None, source_url=None):
    ...
    # Prepare prompt text (same as before)
...
"""
    ...
    if raw_text: prompt_text += f"\n\nOriginal Input Text:\n---\n{raw_text}\n---"
    if source_url: prompt_text += f"\nOriginal Source URL: {source_url}"
    ...
    for attempt in range(max_retries):
        try:
            with gemini_api_semaphore:
                ...
                if file_path and os.path.exists(file_path):
                    ...
                    if mime_type and ('video' in mime_type or 'audio' in mime_type):
                        ...
                        # <<< FIX 1: POLL UNTIL ACTIVE >>>
                        print(f"Uploaded. Waiting for processing (Current State: {uploaded_file_ref.state})...")
                        while uploaded_file_ref.state == "PROCESSING":
                            time.sleep(2)
                            uploaded_file_ref = client.files.get(name=uploaded_file_ref.name)
                        
                        if uploaded_file_ref.state != "ACTIVE":
                            raise ValueError(f"File processing failed. State: {uploaded_file_ref.state}")
                            
                        print("File is ACTIVE. Sending prompt...")
                        prompt_contents.append(uploaded_file_ref)
                ...
                # <<< FIX 2: CORRECT CLEANUP SYNTAX >>>
                if uploaded_file_ref:
                    try:
                        client.files.delete(name=uploaded_file_ref.name)
                        print(f"Cleaned up Gemini file: {uploaded_file_ref.name}")
                    except Exception as e:
            ...
            print(f"ERROR [Gemini Attempt {attempt}]: {e}")
...
def download_media_from_url(url, task_type='video'):
    ...
    # We use a placeholder extension; yt-dlp will fill in the real one
    ...
    if task_type == 'video':
        ...
        # We use 'best[ext=mp4]/best' to get the best single file. 
        # We REMOVED 'merge_output_format' to avoid needing FFmpeg.
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            # yt-dlp generates the filename based on the extension of the download
            downloaded_file = ydl.prepare_filename(info)
            
            # Special handling for audio where we force conversion to wav (requires ffmpeg, but falling back is hard)
            if task_type == 'audio':
                base, _ = os.path.splitext(downloaded_file)
                if os.path.exists(base + '.wav'):
                    downloaded_file = base + '.wav'

            # Double check the file exists, as sometimes extensions differ from prediction
            if not os.path.exists(downloaded_file):
                # Try common fallback extensions
                base, _ = os.path.splitext(downloaded_file)
                for ext in ['.mp4', '.mkv', '.webm', '.wav']:
                    if os.path.exists(base + ext):
                        downloaded_file = base + ext
                        break
                
                if not os.path.exists(downloaded_file):
                    return None, None, f"Download appeared to succeed, but file not found: {url}"
            
            # Create a nice filename for the UI
            original_filename = secure_filename(info.get('title', 'downloaded_media')[:50])
            if not original_filename:
                original_filename = "downloaded_video"
            original_filename += f".{os.path.basename(downloaded_file).split('.')[-1]}"
        ...
        clean_error_message = ansi_escape.sub('', str(e))
        return None, None, f"Download failed. Error: {clean_error_message}"
...
def generate_video_analysis_stream(current_user, video_path, source_identifier):
        ...
        def upload(self, path):
            try:
                if client:
                    ...
                    # FIXED: Use 'file=' instead of 'path='
                    self.media_file = client.files.upload(file=path)
                    print(f"Parallel Gemini upload finished: {self.media_file.name}")
                else:
                    print("Skipping upload: Client not initialized.")
            except Exception as e:
                self.error = e
    ...
    try:
        ...
        # If the video model says REAL, we trust it and force the audio verdict to be REAL too.
        if final_result.get('verdict') == 'REAL':
            print("--- Applying override rule: Video is REAL, forcing Audio to REAL. ---")
            final_result['audio_verdict'] = 'REAL'
            # Also reset the confidence to avoid showing a misleading high "fake" confidence for audio
            if 'audio_confidence' in final_result:
                final_result['audio_confidence'] = 0.0
        ...
        if gemini_upload_thread:
            gemini_upload_thread.join(timeout=60)
            if uploader.error:
                print(f"Gemini upload background error: {uploader.error}")
        ...
        # Cleanup
        if gemini_media_file and client:
            try:
                # NEW/CORRECT:
                client.files.delete(name=gemini_media_file.name)
...
# ...
# ... The rest of the functions (login, register, ext_login, etc.) remain exactly the same.
# ...
